...
title_capture=scratch/screenshots/sdl3-gpu-title.png
title_log=scratch/logs/sdl3-gpu-title.log
...
grep -F "windowless run: 0 SDL windows" "$title_log" || exit 1
...
echo "=== running SDL3 GPU scene/UI/fade consumer ==="
runtime_capture=scratch/screenshots/live-sdl3-gpu.png
runtime_log=scratch/logs/live-sdl3-gpu.log
./build/mana --room M0001_00_00 --screenshot "$runtime_capture" \
  --fade-test --warmup 30 --no-audio >"$runtime_log" 2>&1 || exit 1
grep -F "video driver: offscreen" "$runtime_log" || exit 1
grep -F "runtime driver:" "$runtime_log" || exit 1
grep -F "windowless run: 0 SDL windows" "$runtime_log" || exit 1
grep -F "wrote $runtime_capture" "$runtime_log" || exit 1
grep -F "audio decoded 0 sounds / 0 frames" "$runtime_log" || exit 1
if [ ! -s "$runtime_capture" ]; then
  echo "FATAL: live SDL3 GPU consumer wrote no bytes to $runtime_capture"
  exit 1
fi
echo "  running scene/UI/fade wrote pixels with 0 SDL windows and 0 audio frames"
...
(
  restore_corpus_member() {
    tools/clean_verify_outputs.sh restore-corpus-member
  }
...
(
  trap 'tools/clean_verify_outputs.sh remove-corpus-extra' EXIT INT TERM
  : > "$corpus_extra"
  if python3 tools/asset/mpk.py "$archive" --check-dir scratch/dump \
      >"$corpus_log" 2>&1; then
    exit 1
...
# is read-only: compare every generated byte with the checked-in artifact.
  ...
  tools/clean_verify_outputs.sh generated-tables
