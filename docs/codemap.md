...
| Game loop + player | **Boots into the engine's new-game room (`M0001_00_00`) with no arguments**; real-time loop, WASD/arrow movement, floor + wall collision, WAIT/WALK, attack on Space/Z. Room `Init()` is a coroutine and room replacement preserves scenario globals while clearing transient locks. `mapjump` and room-edge loads preserve room-local actor coordinates, including elevated arrival-box floor ownership. `--opening-story` is fixed-step uncapped, silent, SDL-offscreen, and selects the default Warrior regimen when progression requires input; non-capture windowless runs skip rendering but retain simulation. The route instrument uses strict authored XYZ box containment, validates compressed segments against shipping wall/floor movement, emits exact lattice fallbacks, preserves measured exit-contact coordinates, stages vertical goals, handles the visible 30-unit Kett steps, and exits nonzero with explicit reachability evidence when no route exists. Auto-combat logs every target transition and uses 3D range, so stacked-floor enemies cannot cause an endless rejected swing. The continuous unseeded run crosses the post-Matock cave, completes Kett's kidnapping scene, leaves the manor, defeats five lizardmen, levels up, and acquires the Silver Key | `src/host/main.cpp`, `src/engine/script.cpp`, `src/engine/world.cpp` |
