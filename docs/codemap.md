...
| Desktop host (window + GLES2) | **Textured characters + rooms; GPU skinning + per-actor `.smot` playback.** Actors created by a running room coroutine load their renderable and motion assets on demand instead of remaining invisible. Architecture is actively being migrated toward the Dusklight reference: `main.cpp` remains legacy debt, while interaction, deterministic story/shop policy, and checked PNG output now have cohesive host owners and a mechanical no-growth gate | `src/host/main.cpp`, `src/host/interaction.{h,cpp}`, `src/host/story_driver.{h,cpp}`, `src/host/image_write.{h,cpp}`, `tools/check_structure.py` |
