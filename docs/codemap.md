...
| SDL3 GPU foundation | **Live device, portable embedded shader pack, graphics pipeline, and synchronized offscreen readback.** The mandatory test selects the available backend, uses offscreen video with zero SDL windows and no audio, discriminates exact black/magenta clears across 24 pixels, and draws a full-target triangle across 48/48 pixels. Separate wrong-color negatives reject 12/12 clear pixels and 48/48 pipeline pixels. Two HLSL sources regenerate byte-exact SPIR-V, DXIL, and normalized MSL artifacts; a missing six-artifact pack fails loudly. Scene assets, UI, and presentation migration remain missing; the shipping game path is still GLES2 | `src/host/gpu_device.{h,cpp}`, `src/host/gpu_pipeline.{h,cpp}`, `src/tools/gpu_selftest.cpp`, `shaders/`, `tools/bootstrap_shadercross_linux.sh`, `tools/compile_shaders.sh`, `tools/embed_shader_pack.py`, `tools/normalize_shader_text.py`, `docs/renderer-architecture.md` |
