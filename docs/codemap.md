...
| Title screen | **Ported and rendering.** Boot chain runs Init -> CESA -> MakerLogo -> Title -> Game. The attract screen and the three-item menu are the engine's own: the item ids come from the table `ModeTitle::Render` memcpys from `0xbd354`, in its order, and every word is the shipping string table's. Continue/Load Game are shown dimmed (no save format past the inventory). Name entry and the opening crawl are NOT drawn yet | `src/engine/mode.{h,cpp}`, `src/host/main.cpp` |
