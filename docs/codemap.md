...
| SDL3 GPU foundation | **Live device, portable embedded shader pack, GPU asset owner, depth-tested/material-blended static pipeline, and synchronized offscreen readback.** The mandatory tests select the available backend, create zero SDL windows and initialize no audio, discriminate two exact clears and a full-target triangle, then load shipping room `M0001_00_00`, upload its buffers/textures, and submit 12 draw ranges. All 3,868 non-clear pixels differ from its forced-white texture render; a later far white redraw changes 0 with depth and 3,868 without it. Shipping room `M0000_00_03` supplies two authored water draws and differs from the opaque control in 128/6,912 pixels. Four HLSL sources regenerate byte-exact SPIR-V, DXIL, and normalized MSL artifacts (12 total); a missing pack fails loudly. Shipping scene integration, skinning, UI, and presentation remain missing; the game path is still GLES2 | `src/host/gpu_device.{h,cpp}`, `src/host/gpu_shader.{h,cpp}`, `src/host/gpu_asset.{h,cpp}`, `src/host/gpu_asset_pipeline.{h,cpp}`, `src/tools/gpu_{,asset_}selftest.cpp`, `shaders/`, `tools/compile_shaders.sh`, `docs/renderer-architecture.md` |
