...
| Room data tables (`.odt`/`.gdt`/`.edt`) | **Layouts from the engine's own loaders; 1328/1328 files parse.** The C++ `.gdt` reader serves the shipping 7.5-unit ground-attribute grid to Lua. Room extent comes from **the engine's own world table** (988 of 993 rooms; `.gdt` and collision bounds are fallbacks). Map objects render in-game (3284/3284 ids resolve); `.odt +0x3c` flags and `+0x40` script ids are retained. The binary-derived `0x08` breakable class has live AABB contact, Matock weapon-kind-6 destruction/removal, and dispatches its shipping `_BREAKOBJ_<script-id>` callback. `ObjVisible` controls the same placed object in rendering and collision. Collision policy/shapes for the other object flag classes remain undecoded | `tools/asset/roomdata.py`, `tools/asset/object_table.py`, `src/mcf/assets.cpp`, `src/host/main.cpp` |
| Title screen | **Ported with backend-independent sprite/text frames consumed by GLES and SDL3 GPU.** Boot chain runs Init -> CESA -> MakerLogo -> Title -> Game. The attract screen and three-item menu use the engine's table/string ids; Continue/Load Game remain dimmed because no trustworthy save format exists. Name entry uses the engine's validation/errors. The 15-line opening crawl retains authored offsets, fades, scroll rate, and Skip. The verifier captures the running offscreen ModeTitle with zero audio frames; SDL3 omission controls observe 34,625 sprite and 10,253 text pixels. Interactive presentation still uses the GLES window | `src/engine/mode.{h,cpp}`, `src/host/title_ui.{h,cpp}`, `src/host/render_sprite.{h,cpp}`, `src/host/gpu_sprite.{h,cpp}`, `src/host/main.cpp` |
...
| C++ asset layer | **DONE for MPK/.stex/.smdl.** `RenderAsset` owns backend-independent model/texture retention, material-texture resolution, bounds, and area-weighted normals derived from indexed shipping geometry; both GLES and SDL3 GPU consume the asset boundary | `src/mcf/`, `src/host/render_asset.{h,cpp}`, `src/host/render_normals.{h,cpp}` |
| Desktop host (window + GLES2) | **Textured characters + rooms; GPU skinning + per-actor `.smot` playback.** Running GLES consumes backend-independent scene, game UI, title UI, and sprite frames. The transitional `gles::Asset` now exclusively owns GLES texture/buffer uploads instead of polluting backend-independent actor-model policy. Architecture follows the Dusklight ownership lead: cohesive modules own interaction, deterministic story/shop policy, camera tracking, scene/UI/sprite data, image output/comparison, and paired capture; the mechanical no-growth gate ratcheted legacy `main.cpp` down to 7,070 lines | `src/host/main.cpp`, `src/host/gles_asset.{h,cpp}`, `src/host/interaction.{h,cpp}`, `src/host/story_driver.{h,cpp}`, `src/host/render_{camera,snapshot,ui,sprite}.{h,cpp}`, `src/host/{game_ui_content,title_ui,gles_ui_renderer}.{h,cpp}`, `src/host/image_{compare,write}.{h,cpp}`, `src/host/scene_pair_capture.{h,cpp}`, `tools/check_structure.py` |
| SDL3 GPU foundation | **Live device, portable embedded shader pack, GPU asset/scene/running-snapshot/frame-compositor/UI/sprite/fade owners, target-independent submission, depth-tested/material-blended static pipeline, two-bone skinned pipeline, generated surface normals, named directional lighting, synchronized offscreen readback, and isolated presentation ownership.** Mandatory tests create zero windows and initialize no audio. Shipping scene, skinning, lighting, font/UTF-8 UI, title sprite/text, and fade all have positive and omission/wrong-output controls. The shared frame compositor owns scene -> UI -> authored-fade ordering for both deterministic capture and caller-owned presentation passes. Presentation owns claim/release, resize-dependent RGBA/depth targets, format-converting swapchain blit, and immediate/vsync selection; its actual-window smoke is explicit and excluded from the windowless normal gate. Eleven HLSL sources regenerate byte-exact SPIR-V, DXIL, and normalized MSL artifacts (33 total); a missing pack fails loudly. Interactive rendering still uses GLES2 because the running world cache remains typed as the now-isolated `gles::Asset` owner rather than plain `RenderAsset` | `src/host/gpu_device.{h,cpp}`, `src/host/gpu_presentation.{h,cpp}`, `src/host/gpu_shader.{h,cpp}`, `src/host/gpu_{asset,asset_pipeline,scene,snapshot_renderer,frame_renderer,ui,sprite,overlay,upload}.{h,cpp}`, `src/host/render_{lighting,normals,pose,overlay,ui,sprite}.{h,cpp}`, `src/tools/{render_normals,gpu_asset,gpu_ui}_selftest.cpp`, `shaders/`, `tools/compile_shaders.sh`, `docs/renderer-architecture.md` |
| Verification executables | **Focused native discriminators for generated normals plus SDL3 GPU device, pipeline, and shipping assets.** They are mandatory inputs to `tools/verify.sh`, run offscreen/silent, and are not alternate game entry points | `src/tools/`, `src/tools/render_normals_selftest.cpp`, `src/tools/gpu_selftest.cpp`, `src/tools/gpu_asset_selftest.cpp` |
| Lua bridge | **Lua 5.3 + all 200 `cmd` bindings; 714/715 scripts run.** `AddEnemyZaco` implements the binary's `(count, ids terminated by -1)` contract. `AddBox` creates the binary's `_BOX` actor with its four-slot payload/open-state semantics; opening one publishes its selected item as `tmp_tresureitem` before invoking the shipping `_BOX` callback, including the no-handler case. `OpenDoor` clears the authored directional room mark; and Lua item add/check/delete calls share the live `Inventory`. `GetEquipID` uses the binary's eight-slot bounds and live equipped-item state. `ShopInit`/`ShopAdd` preserve authored stock, `ItemPriceBuy`/`ItemPriceSell` use a generated exact binary table, and `GetRC`/`AddRC` share authoritative player money; host selection policies can drive the shipping shop coroutine headlessly while the interactive shop screen remains missing. `ObjVisible`/`ObjIsVisible` share per-room state, so hidden-floor scripts no longer branch on a generic false stub | `src/engine/script.cpp` |
| Actor system | **Live actors driven by scripts**, rendered and animated in their rooms. `ChrMoveTo`/`ChrMoveYTo` use the shipping `(speed,x[,y],z)` contract and advance asynchronously. Motion commands synchronously resolve the actor's real `.smot` duration, then frame/end/finish queries drive Lua and rendering from the same per-actor clock. MainPlayer script HP/MAXHP/MP/MAXMP access shares authoritative `PlayerStats` with combat instead of reading Actor's presentation copy; the shipping `_HEALSPRING` coroutine is tested against both damaged and full states. `ChrLookTarget`, the exported `math_LerpSin`/`math_atan2`/`bit_and` helpers, and the opening Jackal's real scripted movement path are active. Engine-chosen chip placement is repeatable after coroutine resumes, so `Init`-spawned random enemies receive distinct walkable positions in the same frame. `AddParty` now uses `ModeGame`'s binary-derived persistent id and exact `PARTY_HEROINE`..`PARTY_MARCIE` handle table; the companion is restored under that script-visible handle after every room load. Tagged cutscene actors created through `npc()` decode `100+eENEMY` and `1000+eBOSS` into the E/B model namespaces; Julius and the Shadow Knight render and complete their shipping motions. `--movement-selftest` covers 78 cases including player/shop state ownership, interaction and story-target selection positives and negatives, shipping actor-model exceptions, treasure callback payloads, the healing spring, event-box floor resolution, route-height transitions, temporary mapjump-floor ownership, doors, switch payloads, object visibility, inventory/boxes, equipment queries, party naming, idempotence, and removal. Party follow AI and scripted movement route tables remain missing | `src/engine/world.{h,cpp}`, `src/engine/event_box.{h,cpp}`, `src/engine/script.cpp`, `src/host/interaction.{h,cpp}`, `src/host/navigation.{h,cpp}`, `src/host/story_driver.{h,cpp}`, `src/host/main.cpp`, `src/host/render.cpp` |
| Game loop + player | **Boots into the engine's new-game room (`M0001_00_00`) with no arguments**; real-time loop, WASD/arrow movement, floor + wall collision, WAIT/WALK, attack on Space/Z, and binary-derived Cure on C. Room `Init()` is a coroutine and room replacement preserves scenario globals while clearing transient locks. `mapjump` and room-edge loads preserve room-local actor coordinates, including strict-overlap elevated arrival-box floor ownership; exact body tangency does not steal an adjacent box's floor. Padded 330x270 rooms cross their 15-unit format margin plus the measured 30-unit room-edge body radius. `--opening-story` is fixed-step uncapped, silent, SDL-offscreen, and selects the default Warrior regimen when progression requires input; non-capture windowless runs skip rendering but retain simulation. An unmet `--stop-room` exits nonzero and prints both requested and final rooms. The route instrument uses strict authored XYZ box containment, blocked character-body event contact, explicit lower actor goals, and floor-type-1 X/Y wall-plane movement while adopting scripted character teleports. It sweeps edges from the real lattice node at shipping fixed-step distance, preserves exact BFS edges across floor changes, distinguishes components at the shipping 30-unit step boundary, validates descending event routes from the live first point at each shipping movement sample, emits exact diagnostics after 120 blocked frames—including locked doors after their route deque empties—preserves measured exit-contact coordinates, stages upward goals only, and handles the visible Kett steps. Temporary mapjump-volume floor ownership no longer rebuilds an identical point-ground route at every waypoint. Pressure-switch boxes receive their pressed callback payload instead of silently taking the released branch. The continuous unseeded run reaches and defeats Hydra, opens Fire then Mirror, runs `AfterBossEvent`, returns south and west across the connected overworld route, exposes the Butler with equipped Mirror, defeats the `AddEnemy(123)` Steward Wolf using the shipping `B0023_00` model exception, traverses Kett's hidden-floor route, opens the Chain Flail chest through the shipping callback, and settles at `sccnt=20` | `src/host/main.cpp`, `src/host/story_driver.{h,cpp}`, `src/host/navigation.{h,cpp}`, `src/engine/script.cpp`, `src/engine/world.cpp`, `src/host/render.cpp` |
| Camera | **Script-driven camera with a backend-independent tracker**: `eCamGetData` orbit slots plus distinct fixed target, target offset, character target and explicit eye-position commands. `--camera-selftest` executes six Lua-command cases, seven tracker cases (including the missing-character fallback and exact one-step interpolation), and three render-snapshot cases. The opening frame shows the arena gate instead of the back of its wall. Camera collision/occluder handling remains missing | `src/engine/world.h`, `src/engine/script.cpp`, `src/host/render_camera.{h,cpp}`, `src/host/render_snapshot.{h,cpp}`, `src/host/main.cpp` |
...
| Fade | **Timed engine fades consumed by GLES and a dedicated SDL3 GPU composition pass.** The half-black positive and zero-coverage negative check RGB and destination alpha; the running paired capture proves authored 0.500 coverage offscreen | `src/engine/world.h`, `src/host/render_overlay.{h,cpp}`, `src/host/gpu_overlay.{h,cpp}`, `src/host/scene_pair_capture.{h,cpp}` |
...
| Event boxes + transitions | **Edge-triggered boxes run handlers as coroutines; `mapjump` loads the destination room.** One callback name can own multiple physical volumes (all retain their callback and name-wide setters update all matches). `AddEventBox` sentinel-floor probing and its lower-bound-minus-one rule are centralized in `EventBox`; containment is the engine's strict enabled 3D test. A static-collision-blocked movement attempt can contact an ordinary box with the character body without broadening all live event tests. Paired `WALL_UP`/`WALL_DN` volumes drive climbable vines, while authored lone `WALL_UP` terrain regions enter floor-type-1 wall movement. KEY doors consume equipped key id 18, 30, or 37 from button slots 4..7 | `src/engine/event_box.{h,cpp}`, `src/engine/world.h`, `src/engine/script.cpp`, `src/host/main.cpp` |
| Audio | **BGM + SE play, driven by scripts** | `src/engine/audio.cpp` |
| Status HUD | **HP/MP, GP/EXP, ATK/DEF/Lv drawn in the corner through one backend-independent UI frame consumed by GLES and SDL3 GPU**, with labels taken from the game's own `SYS_COMMON_STATUS_LABEL_*` strings so it follows `--lang`. Layout is a PORT CHOICE — `ModeGame::Draw_StatusData` is not reversed, so this is a plain readout rather than a fake of the game's UI. `--no-hud` hides it | `src/host/game_ui_content.{h,cpp}`, `src/host/render_ui.{h,cpp}`, `src/host/{gles_ui_renderer,gpu_ui}.{h,cpp}` |
| Text / dialogue | **NPC conversations work in-game, with the control codes expanded** (`CnvFormatString`: `@N(n)` -> a `CHARACTER_NAME_n`, `@H`/`@G` -> the hero/girl names, `@P @i @I @S` -> the script-set parameter slots; 392 of 393 strings expand fully, `--text-selftest`) — walk up, press Space, lines advance one at a time. Drawn on screen and waits for the player (`msgId`'s `coroutine.yield` is honoured). 146 lines fire across the shipping scripts with 0 unresolved ids. — 1906 strings in en and ja, rendered in the game's own `font_<lang>.bin` with decoded-codepoint word wrap and metric-derived line pitch through the shared GLES/SDL3 UI frame — 543 glyphs in English and 1209 in Japanese. `BasicFont.sfont` (ASCII 32..126) remains only as the fallback | `tools/asset/strings.py`, `src/mcf/assets.cpp`, `src/host/render_ui.{h,cpp}`, `src/host/{gles_ui_renderer,gpu_ui}.{h,cpp}` |
...
- ~~19 of 53 NPC ids have no model?~~ **ANSWERED: the mapping was wrong, and
  ...
  armour). Ordinary enemies and bosses are id-for-id (73/73 and 23/23), with
  one measured shipping exception: `ModeGame::AddEnemy` @ `0x2dda74` replaces
  enemy id 123's normal `E0123_00` name with literal `B0023_00` for the Steward
  Wolf transformation.
...
Needs SDL3 and GLESv2; lucent is fetched by CMake. SDL3 GPU is exercised by a
separate windowless device/readback target while the shipping renderer remains
GLES2. `--screenshot` renders one
...
reports with denominators. Current state:
    ...
    214 actors spawned, 0 with no model, 8 intentionally invisible (eNPC.TRANS)
    539 event boxes
