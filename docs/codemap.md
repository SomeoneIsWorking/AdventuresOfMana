...
| Desktop host (window + GLES2) | **Textured characters + rooms; GPU skinning + per-actor `.smot` playback.** Actors created by a running room coroutine load their renderable and motion assets on demand instead of remaining invisible. Architecture is actively being migrated toward the Dusklight reference: `main.cpp` remains legacy debt, while interaction and deterministic story/shop policy now have cohesive host owners and a mechanical no-growth gate | `src/host/main.cpp`, `src/host/interaction.{h,cpp}`, `src/host/story_driver.{h,cpp}`, `tools/check_structure.py` |
| SDL3 GPU foundation | **Live device, portable embedded shader pack, GPU asset owner, depth-tested/material-blended static pipeline, two-bone skinned pipeline, and synchronized offscreen readback.** The mandatory tests select the available backend, create zero SDL windows and initialize no audio, discriminate two exact clears and a full-target triangle, then load shipping room `M0001_00_00`, upload its buffers/textures, and submit 12 draw ranges. All 3,868 non-clear pixels differ from its forced-white texture render; a later far white redraw changes 0 with depth and 3,868 without it. Shipping room `M0000_00_03` supplies two authored water draws and differs from the opaque control in 128/6,912 pixels. Shipping hero `C0000_00` renders 2,812 non-clear pixels through the shared 80-bone palette, changes 3,723 pixels when every joint is translated, and rejects a missing palette. Five HLSL sources regenerate byte-exact SPIR-V, DXIL, and normalized MSL artifacts (15 total); a missing pack fails loudly. Shipping scene integration, UI, and presentation remain missing; the game path is still GLES2 | `src/host/gpu_device.{h,cpp}`, `src/host/gpu_shader.{h,cpp}`, `src/host/gpu_asset.{h,cpp}`, `src/host/gpu_asset_pipeline.{h,cpp}`, `src/host/render_pose.{h,cpp}`, `src/tools/gpu_{,asset_}selftest.cpp`, `shaders/`, `tools/compile_shaders.sh`, `docs/renderer-architecture.md` |
