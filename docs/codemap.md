...
| Event boxes + transitions | **Edge-triggered boxes run handlers as coroutines; `mapjump` loads the destination room.** Engine containment is now matched: `AppEventBoxBase::IsHit` is a strict enabled 3D box test (not the port's former inclusive XZ test), and no-touch boxes cannot fire | `src/engine/world.h`, `src/engine/script.cpp`, `src/host/main.cpp` |
