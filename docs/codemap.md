...
| SDL3 GPU foundation | **Live device, portable embedded shader pack, GPU asset/scene/running-snapshot owners, target-independent submission, depth-tested/material-blended static pipeline, two-bone skinned pipeline, and synchronized offscreen readback.** The mandatory tests select the available backend, create zero SDL windows and initialize no audio, discriminate two exact clears and a full-target triangle, then load shipping room `M0001_00_00`, upload its buffers/textures, and submit 12 draw ranges. All 3,868 non-clear pixels differ from its forced-white texture render; a later far white redraw changes 0 with depth and 3,868 without it. Shipping room `M0000_00_03` supplies two authored water draws and differs from the opaque control in 128/6,912 pixels. Shipping hero `C0000_00` renders 2,812 non-clear pixels through the shared 80-bone palette, changes 3,723 pixels when every joint is translated, and rejects a missing palette. The scene owner submits every asset's opaque ranges before any blended ranges: 748/6,912 pixels discriminate that order from per-asset submission. Its snapshot adapter reproduces the manually assembled scene with 0/76,800 pixels different; rendering that snapshot into a caller-owned command/render pass differs from its readback wrapper in 0/76,800 pixels, while missing targets and unnamed assets are rejected. The running room captures one frame through GLES and SDL3 GPU from the same 3-instance snapshot, offscreen and with zero audio frames. Five HLSL sources regenerate byte-exact SPIR-V, DXIL, and normalized MSL artifacts (15 total); a missing pack fails loudly. UI and presentation remain missing; interactive rendering still uses GLES2 | `src/host/gpu_device.{h,cpp}`, `src/host/gpu_shader.{h,cpp}`, `src/host/gpu_asset.{h,cpp}`, `src/host/gpu_asset_pipeline.{h,cpp}`, `src/host/gpu_scene.{h,cpp}`, `src/host/gpu_snapshot_renderer.{h,cpp}`, `src/host/render_pose.{h,cpp}`, `src/tools/gpu_{,asset_}selftest.cpp`, `shaders/`, `tools/compile_shaders.sh`, `docs/renderer-architecture.md` |
