...
| SDL3 GPU foundation | **Live, portable device owner with synchronized offscreen render-target readback.** The mandatory selftest requests every SDL GPU shader format, selects the available backend, uses the offscreen video driver with zero SDL windows, never initializes audio, and discriminates exact black and magenta clears across all 24 pixels. Its wrong-color negative proves all 12 magenta pixels are rejected. Scene, asset, UI, and presentation migration remain missing; the shipping game path is still GLES2 | `src/host/gpu_device.{h,cpp}`, `src/tools/gpu_selftest.cpp`, `docs/renderer-architecture.md` |
