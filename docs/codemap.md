...
| Desktop host (window + GLES2) | **Textured characters + rooms; GPU skinning + per-actor `.smot` playback.** Actors created by a running room coroutine load their renderable and motion assets on demand instead of remaining invisible. The running GLES draw consumes backend-independent scene and UI frames instead of resolving mutable `World` state during submission. Architecture follows the Dusklight ownership lead: `main.cpp` remains legacy debt, while interaction, deterministic story/shop policy, camera tracking, scene/UI snapshots, image comparison/output, and paired-backend capture have cohesive owners; the mechanical no-growth gate ratcheted `main.cpp` down to 7,223 lines | `src/host/main.cpp`, `src/host/interaction.{h,cpp}`, `src/host/story_driver.{h,cpp}`, `src/host/render_camera.{h,cpp}`, `src/host/render_{snapshot,ui}.{h,cpp}`, `src/host/game_ui_content.{h,cpp}`, `src/host/gles_ui_renderer.{h,cpp}`, `src/host/image_{compare,write}.{h,cpp}`, `src/host/scene_pair_capture.{h,cpp}`, `tools/check_structure.py` |
| SDL3 GPU foundation | **Live device, portable embedded shader pack, GPU asset/scene/running-snapshot/UI/fade owners, target-independent submission, depth-tested/material-blended static pipeline, two-bone skinned pipeline, generated surface normals, named directional lighting, and synchronized offscreen readback.** The mandatory tests select the available backend, create zero SDL windows and initialize no audio, then load shipping room `M0001_00_00`, derive normals from 1,094 triangles with zero degenerate/unsupported vertices, upload its buffers/textures, and submit 12 draw ranges. Enhanced lighting differs from vanilla in all 3,868 drawn pixels and from an equal-ambient control in 3,675, proving directional shading rather than global dimming. Shipping hero `C0000_00` differs from its equal-ambient control in 2,750 pixels through the shared 80-bone palette. The verifier writes distinct enhanced/vanilla A/B captures. Depth, blending, scene-wide material order, snapshot/caller-owned-pass identity, shipping-font UI/UTF-8 layout, authored fade composition, and all negative controls remain mandatory. Nine HLSL sources regenerate byte-exact SPIR-V, DXIL, and normalized MSL artifacts (27 total); a missing pack fails loudly. Presentation and boot/title UI remain missing; interactive rendering still uses GLES2 | `src/host/gpu_device.{h,cpp}`, `src/host/gpu_shader.{h,cpp}`, `src/host/gpu_{asset,asset_pipeline,scene,snapshot_renderer,ui,overlay,upload}.{h,cpp}`, `src/host/render_{lighting,normals,pose,overlay,ui}.{h,cpp}`, `src/tools/{render_normals,gpu_asset,gpu_ui}_selftest.cpp`, `shaders/`, `tools/compile_shaders.sh`, `docs/renderer-architecture.md` |
