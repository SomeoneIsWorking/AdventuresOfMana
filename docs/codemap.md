...
| Game loop + player | **Boots into the engine's new-game room (`M0001_00_00`) with no arguments**; real-time loop, WASD/arrow movement, floor + wall collision, WAIT/WALK, attack on Space/Z. Room `Init()` runs as its own coroutine, advances from the engine-style game clock used by `wait`, and is cleared on map change without losing scenario globals. `eventScene`, `SetCinema`, and `SetPlayerControllEnable` gate human and headless input; transient fade locks reset at room replacement, so cutscenes cannot be escaped early or strand the destination. `mapjump` and room-edge loads preserve the room-local actor-coordinate invariant. Manual and authored scripted movement share world-table edge/door resolution, so out-of-room `PlMoveToChip` destinations transition rather than strand the player. The mandatory uncapped, silent `--opening-story` gate runs continuously from a new game through **both distinct Jackal fights**, Will's scene, the Julius/Shadow confrontation, chase, attack and waterfall recovery. A second mandatory continuation follows the real overworld/vine route and fires `M0000_06_05`'s `in_01`, reaching Bogard's house `M0010_00_01` with `sccnt=10` and zero decoded audio | `src/host/main.cpp`, `src/engine/script.cpp`, `src/engine/world.cpp` |
...
| Event boxes + transitions | **Edge-triggered boxes run handlers as coroutines; `mapjump` loads the destination room.** One callback name can own multiple physical volumes (all retain their callback and name-wide setters update all matches). Containment is the engine's strict enabled 3D test, made at the player centre 15 units above its collision-floor contact; no-touch boxes cannot fire. Special `WALL_UP`/`WALL_DN` pairs drive climbable vines as edge-triggered character-volume contacts; the mandatory Bogard run climbs three authored floors (0→90→150→180), crosses from an out-of-room vine destination, and fires the elevated `in_01`. Ordinary room-edge loads move one character radius inward and use the engine's height-bounded floor selection on stacked terrain | `src/engine/world.h`, `src/engine/script.cpp`, `src/host/main.cpp` |
