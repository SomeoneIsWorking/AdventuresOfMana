...
| Title screen | **Ported through the shipping SDL3 GPU runtime.** Boot chain runs Init -> CESA -> MakerLogo -> Title -> Game. The attract screen and three-item menu use the engine's table/string ids; Continue/Load Game remain dimmed because no trustworthy save format exists. Name entry uses the engine's validation/errors. The 15-line opening crawl retains authored offsets, fades, scroll rate, and Skip. The verifier captures the running ModeTitle through the same GPU runtime with zero SDL windows and zero audio frames; omission controls observe 34,625 sprite and 10,253 text pixels | `src/engine/mode.{h,cpp}`, `src/host/title_ui.{h,cpp}`, `src/host/render_sprite.{h,cpp}`, `src/host/gpu_{runtime_renderer,sprite}.{h,cpp}`, `src/host/main.cpp` |
...
| C++ asset layer | **DONE for MPK/.stex/.smdl.** `RenderAsset` is the sole CPU authority for model/texture retention, material-texture resolution, bounds, and area-weighted normals; the SDL3 GPU cache uploads it lazily by shipping asset name | `src/mcf/`, `src/host/render_asset.{h,cpp}`, `src/host/render_normals.{h,cpp}`, `src/host/gpu_snapshot_renderer.{h,cpp}` |
| Desktop host | **SDL3 GPU is the default game/title renderer; GLES2 is isolated to the explicit single-model inspector.** `gpu_runtime_renderer` owns the shipping device plus optional presentation, while `gpu_frame_renderer` composes immutable scene -> UI -> fade inputs for both swapchain presentation and texture readback. The transitional GLES game cache/UI/pair owners were removed. Architecture follows the Dusklight ownership lead: cohesive owners cover presentation, rendering, interaction, story policy, camera, UI, images, and diagnostics. The structure gate ratcheted legacy `main.cpp` down to 6,819 lines and caps all new C++ modules at 1,000 | `src/host/gpu_{runtime_renderer,presentation,frame_renderer,snapshot_renderer}.{h,cpp}`, `src/host/interaction.{h,cpp}`, `src/host/story_driver.{h,cpp}`, `src/host/render_{camera,snapshot,ui,sprite}.{h,cpp}`, `src/host/{game_ui_content,title_ui,image_write}.{h,cpp}`, `tools/check_structure.py` |
| SDL3 GPU renderer | **Shipping and verified.** Live device, embedded portable shaders, GPU asset/scene/frame/UI/sprite/fade owners, target-independent submission, depth-tested/material-blended static geometry, two-bone skinning, generated normals, directional lighting, synchronized readback, and resize-safe presentation all serve the running game. Normal tests and live captures create zero SDL windows and decode zero audio frames; non-capture story tests skip graphics entirely and run fixed-step without wall-clock pacing. Focused positives and omission/wrong-output controls cover shipping scene, skinning, lighting, font/UTF-8 UI, title sprite/text, fade, and presentation policy. Eleven HLSL sources regenerate byte-exact SPIR-V, DXIL, and normalized MSL artifacts (33 total) | `src/host/gpu_device.{h,cpp}`, `src/host/gpu_presentation.{h,cpp}`, `src/host/gpu_runtime_renderer.{h,cpp}`, `src/host/gpu_{asset,asset_pipeline,scene,snapshot_renderer,frame_renderer,ui,sprite,overlay,upload}.{h,cpp}`, `src/host/render_{lighting,normals,pose,overlay,ui,sprite}.{h,cpp}`, `src/tools/{render_normals,gpu_asset,gpu_ui}_selftest.cpp`, `shaders/`, `tools/compile_shaders.sh`, `docs/renderer-architecture.md` |
...
| Fade | **Timed engine fades consumed by the shipping SDL3 GPU composition pass after scene and UI.** The half-black positive and zero-coverage negative check RGB and destination alpha; the live runtime capture exercises authored 0.500 coverage without a window | `src/engine/world.h`, `src/host/render_overlay.{h,cpp}`, `src/host/gpu_{overlay,frame_renderer,runtime_renderer}.{h,cpp}` |
...
| Status HUD | **HP/MP, GP/EXP, ATK/DEF/Lv drawn through one backend-independent UI frame consumed by the SDL3 GPU runtime**, with labels taken from the game's own `SYS_COMMON_STATUS_LABEL_*` strings so it follows `--lang`. Layout is a PORT CHOICE — `ModeGame::Draw_StatusData` is not reversed, so this is a plain readout rather than a fake of the game's UI. `--no-hud` hides it | `src/host/game_ui_content.{h,cpp}`, `src/host/render_ui.{h,cpp}`, `src/host/gpu_ui.{h,cpp}` |
| Text / dialogue | **NPC conversations work in-game, with control codes expanded and shipping-font SDL3 GPU rendering.** `CnvFormatString` resolves names and parameter slots; 392 of 393 strings expand fully and 146 lines fire across shipping scripts with 0 unresolved ids. English and Japanese use decoded-codepoint wrapping and metric-derived pitch from `font_<lang>.bin` (543/1209 glyphs); `BasicFont.sfont` remains the ASCII fallback | `tools/asset/strings.py`, `src/mcf/assets.cpp`, `src/host/render_ui.{h,cpp}`, `src/host/gpu_ui.{h,cpp}` |
...
Needs SDL3 and GLESv2; lucent is fetched by CMake. SDL3 GPU is the shipping
game/title renderer. GLESv2 serves only `--model`. `--screenshot` renders through
the shipping GPU frame compositor and exits with zero SDL windows; `--no-window`
story tests skip rendering entirely and remain uncapped and silent.
