...
| Game data tables | **Enemy table live in-game** (`enemydat.bin`, 107 records: max HP, defence, EXP, money each pinned to an engine read) and **`tblWeapon` extracted** (18 records). **`tblItem` decoded** (37 items with names from `ITEM_NAME_<id>`, prices, and the id->category ranges out of `DataTableGetIdType`), `tblHelm`/`tblArmor`/`tblShield`/`tblLevelup` decoded | `src/mcf/assets.cpp`, `docs/assets.md`, `docs/weapon-table.md` |
