...
| World map (which room is in which cell) | **Done and verified.** 32 worlds, 1879 grid cells, 1000 named rooms, read from `.rodata` at `0xbd564`. Every name encodes its own world id and (col,row) (1000/1000); sizes match the `.gdt` ground truth **656/656** against a control that scores 384/656. `SetDoor` state now gates room-edge traversal through adjacent cells: the mandatory live discriminator crosses Will's `UP/FREE` door and proves the destination's `DN/KEY` door does not cross | `tools/asset/worldmap.py`, `src/engine/world_table.inc`, `src/engine/world.{h,cpp}`, `src/host/main.cpp`, `docs/world-map.md` |
