...
| SDL3 GPU foundation | **Live device, portable embedded shader pack, GPU asset/scene/running-snapshot/UI/sprite/fade owners, target-independent submission, depth-tested/material-blended static pipeline, two-bone skinned pipeline, generated surface normals, named directional lighting, synchronized offscreen readback, and isolated presentation ownership.** Mandatory tests create zero windows and initialize no audio. Shipping scene, skinning, lighting, font/UTF-8 UI, title sprite/text, and fade all have positive and omission/wrong-output controls. Presentation owns claim/release, resize-dependent RGBA/depth targets, format-converting swapchain blit, and immediate/vsync selection; its actual-window smoke is explicit and excluded from the windowless normal gate. Eleven HLSL sources regenerate byte-exact SPIR-V, DXIL, and normalized MSL artifacts (33 total); a missing pack fails loudly. Interactive rendering still uses GLES2 because the running world cache remains typed as the now-isolated `gles::Asset` owner rather than plain `RenderAsset` | `src/host/gpu_device.{h,cpp}`, `src/host/gpu_presentation.{h,cpp}`, `src/host/gpu_shader.{h,cpp}`, `src/host/gpu_{asset,asset_pipeline,scene,snapshot_renderer,ui,sprite,overlay,upload}.{h,cpp}`, `src/host/render_{lighting,normals,pose,overlay,ui,sprite}.{h,cpp}`, `src/tools/{render_normals,gpu_asset,gpu_ui}_selftest.cpp`, `shaders/`, `tools/compile_shaders.sh`, `docs/renderer-architecture.md` |
