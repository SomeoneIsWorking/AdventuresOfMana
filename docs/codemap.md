...
| Game loop + player | **Boots into the engine's new-game room (`M0001_00_00`) with no arguments**; real-time loop, WASD/arrow movement, floor + wall collision, WAIT/WALK, attack on Space/Z. Room `Init()` runs as its own coroutine, advances from the engine-style game clock used by `wait`, and is cleared on map change without losing scenario globals. `eventScene`, `SetCinema`, and `SetPlayerControllEnable` gate human and headless input; transient fade locks reset at room replacement, so cutscenes cannot be escaped early or strand the destination. `mapjump` and room-edge loads preserve the room-local actor-coordinate invariant. Manual and authored scripted movement share world-table edge/door resolution, so out-of-room `PlMoveToChip` destinations transition rather than strand the player. `--opening-story` implies fixed-step uncapped execution, disabled audio, and SDL's offscreen driver; windowless non-capture runs skip invisible rendering while retaining simulation, collision, Lua, animation clocks, and combat. Mandatory continuous gates now extend through Bogard's Matock chest and prove item 17 acquisition | `src/host/main.cpp`, `src/engine/script.cpp`, `src/engine/world.cpp` |
