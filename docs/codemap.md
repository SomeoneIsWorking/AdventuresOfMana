...
| Collision format (`.scol`/`SCol`) | **DONE — cells + triangles reversed; floor and XZ wall queries work.** Wall collision uses candidate attribute classes plus the triangle normal because shipping class 1 is mixed; the M0001 gate proves a wall crossing is rejected and open floor is accepted | `tools/asset/scol.py`, `Collision::{GetFloor,BlockedXZ}` |
