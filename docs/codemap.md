...
| Player stats | **`GameParameter` reimplemented from `Init` + `Update`** — level, HP/MP pools, EXP and next-level curve, money, the four stats with their caps, attack from the weapon table and defence from the armour tables. Both derived formulas reproduce `Init`'s own constants (`--player-selftest`). No save/load path, so always a new game at level 1; `tblLevelup` not decoded | `src/mcf/mcf.h`, `docs/assets.md` |
