...
| Game loop + player | **Boots into the engine's new-game room (`M0001_00_00`) with no arguments**; real-time loop, WASD/arrow movement, floor + wall collision, WAIT/WALK, attack on Space/Z. Room `Init()` is a coroutine and room replacement preserves scenario globals while clearing transient locks. `mapjump` and room-edge loads preserve room-local actor coordinates. `--opening-story` is fixed-step uncapped, silent, and SDL-offscreen; non-capture windowless runs skip rendering but retain simulation. The route instrument uses strict authored box containment, validates compressed segments against shipping wall/floor movement, caches across ordinary slopes, invalidates on room/event-wall changes, and refuses irreversible >30-unit floor edges. The mandatory unseeded gate now follows the real two-vine post-Matock route, enters `M0011_00_00`, consumes two of seven Mattock uses against object-id-9 rocks, avoids the room-1 pit, and reaches `M0011_00_02` | `src/host/main.cpp`, `src/engine/script.cpp`, `src/engine/world.cpp` |
