...
| Actor system | **Live actors driven by scripts**, rendered and animated in their rooms. `ChrMoveTo`/`ChrMoveYTo` now use the shipping `(speed,x[,y],z)` contract, advance asynchronously, expose `IsChrAutoMove`, and preserve speed-zero look-at; `--movement-selftest` covers both moving and stationary classes. Scripted movement still uses a straight segment rather than the engine's route table | `src/engine/world.{h,cpp}`, `src/engine/script.cpp`, `src/host/render.cpp` |
