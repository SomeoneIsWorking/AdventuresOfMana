...
| SDL3 GPU foundation | **Live device, portable embedded shader pack, GPU asset/scene/running-snapshot/UI/sprite/fade owners, target-independent submission, depth-tested/material-blended static pipeline, two-bone skinned pipeline, generated surface normals, named directional lighting, and synchronized offscreen readback.** Mandatory tests create zero visible windows and initialize no audio. Shipping scene, skinning, lighting, font/UTF-8 UI, title sprite/text, and fade all have positive and omission/wrong-output controls. Eleven HLSL sources regenerate byte-exact SPIR-V, DXIL, and normalized MSL artifacts (33 total); a missing pack fails loudly. Presentation remains missing; interactive rendering still uses GLES2 | `src/host/gpu_device.{h,cpp}`, `src/host/gpu_shader.{h,cpp}`, `src/host/gpu_{asset,asset_pipeline,scene,snapshot_renderer,ui,sprite,overlay,upload}.{h,cpp}`, `src/host/render_{lighting,normals,pose,overlay,ui,sprite}.{h,cpp}`, `src/tools/{render_normals,gpu_asset,gpu_ui}_selftest.cpp`, `shaders/`, `tools/compile_shaders.sh`, `docs/renderer-architecture.md` |
