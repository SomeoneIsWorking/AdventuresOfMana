...
| C++ asset layer | **DONE for MPK/.stex/.smdl.** `RenderAsset` now owns backend-independent model/texture retention, material-texture resolution, and bounds; both GLES and SDL3 GPU consume it | `src/mcf/`, `src/host/render_asset.{h,cpp}` |
| Desktop host (window + GLES2) | **Textured characters + rooms; GPU skinning + per-actor `.smot` playback.** Actors created by a running room coroutine load their renderable and motion assets on demand instead of remaining invisible. The running GLES draw consumes a backend-independent frame snapshot instead of resolving mutable `World` state during submission. Architecture follows the Dusklight ownership lead: `main.cpp` remains legacy debt, while interaction, deterministic story/shop policy, camera tracking, render snapshots, image comparison/output, and paired-backend capture have cohesive owners; the mechanical no-growth gate ratcheted `main.cpp` down to 7,559 lines | `src/host/main.cpp`, `src/host/interaction.{h,cpp}`, `src/host/story_driver.{h,cpp}`, `src/host/render_camera.{h,cpp}`, `src/host/render_snapshot.{h,cpp}`, `src/host/image_{compare,write}.{h,cpp}`, `src/host/scene_pair_capture.{h,cpp}`, `tools/check_structure.py` |
| SDL3 GPU foundation | **Live device, portable embedded shader pack, GPU asset/scene/running-snapshot/fade owners, target-independent submission, depth-tested/material-blended static pipeline, two-bone skinned pipeline, and synchronized offscreen readback.** The mandatory tests select the available backend, create zero SDL windows and initialize no audio, discriminate two exact clears and a full-target triangle, then load shipping room `M0001_00_00`, upload its buffers/textures, and submit 12 draw ranges. All 3,868 non-clear pixels differ from its forced-white texture render; a later far white redraw changes 0 with depth and 3,868 without it. Shipping room `M0000_00_03` supplies two authored water draws and differs from the opaque control in 128/6,912 pixels. Shipping hero `C0000_00` renders 2,812 non-clear pixels through the shared 80-bone palette, changes 3,723 pixels when every joint is translated, and rejects a missing palette. The scene owner submits every asset's opaque ranges before every blended range. Its snapshot adapter reproduces the manually assembled scene and caller-owned render pass with 0/76,800 pixels different, while missing targets and unnamed assets are rejected. The fade pass changes all 12 focused pixels at half-black coverage and rejects all 12 at zero coverage; the running scene/fade pair uses the same 3-instance snapshot and 0.500 authored coverage, offscreen and with zero audio frames. Seven HLSL sources regenerate byte-exact SPIR-V, DXIL, and normalized MSL artifacts (21 total); a missing pack fails loudly. Text/HUD and presentation remain missing; interactive rendering still uses GLES2 | `src/host/gpu_device.{h,cpp}`, `src/host/gpu_shader.{h,cpp}`, `src/host/gpu_asset.{h,cpp}`, `src/host/gpu_asset_pipeline.{h,cpp}`, `src/host/gpu_scene.{h,cpp}`, `src/host/gpu_snapshot_renderer.{h,cpp}`, `src/host/gpu_overlay.{h,cpp}`, `src/host/render_{pose,overlay}.{h,cpp}`, `src/tools/gpu_{,asset_}selftest.cpp`, `shaders/`, `tools/compile_shaders.sh`, `docs/renderer-architecture.md` |
| Verification executables | **Focused native discriminators for SDL3 GPU device, pipeline, and shipping assets.** They are mandatory inputs to `tools/verify.sh`, not alternate game entry points | `src/tools/`, `src/tools/gpu_selftest.cpp`, `src/tools/gpu_asset_selftest.cpp` |
