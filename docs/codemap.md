...
| Game loop + player | **Boots into the engine's new-game room (`M0001_00_00`) with no arguments**; real-time loop, WASD/arrow movement, floor + wall collision, WAIT/WALK, attack on Space/Z. Room `Init()` runs as its own coroutine, advances from the engine-style game clock used by `wait`, and is cleared on map change without losing scenario globals. `eventScene`, `SetCinema`, and `SetPlayerControllEnable` gate human and headless input; transient fade locks reset at room replacement, so cutscenes cannot be escaped early or strand the destination. `mapjump` and room-edge loads preserve the room-local actor-coordinate invariant. Manual and authored scripted movement share world-table edge/door resolution, so out-of-room `PlMoveToChip` destinations transition rather than strand the player. The mandatory uncapped, silent `--opening-story` gate now runs continuously from a new game through **both distinct Jackal fights**, Will's complete scene, free and ordinary room edges, `out_01`, the complete Julius/Shadow scene, and the two scripted escape edges into `M0001_01_04` with `sccnt=6` | `src/host/main.cpp`, `src/engine/script.cpp`, `src/engine/world.cpp` |
