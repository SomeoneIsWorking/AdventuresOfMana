...
| Player `WALL_UP` / `WALL_DN` contact volume | 15-unit character radius for paired traversal; blocked ordinary event contact uses the measured 30-unit movement body | `AppCharacterBase::_AppEvMove` proves event collision resolves a character against the box, but the precise player collision primitive is not separately reversed. Flag direction, paired volumes, and destination floors are authored: `EvBoxWallUp` stores floor-1 and `EvBoxWallDn` stores floor-14. Lone `WALL_UP` terrain regions change floor type without a fabricated partner; floor type 1 moves on X/Y as proven by `AppCharacterBase::Update` |
...
| KEY door accepted items | equipped item ids 18, 30, or 37 in button slots 4..7; one use consumed | `AppObjectModel::HitCharacter` @ `0x2be160` reads all four button fields, compares exactly these ids, passes the matching button to `ModeGame::UseInventory`, and opens the stored door side |
