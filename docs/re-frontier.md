...
|---|---|---|
| **Enemy AI** | the architecture: a 27-way switch on the AI type sets a movement mode at actor `+0x3934` and a byte at `+0xc7b`, feeding a second switch at `0x2a95b4`. 4 of the 27 cases read, and the mode switch's **dispatch** now mapped — modes 0/1/2 share one body (the one 59 of 107 enemies reach), 3–5 another, 6–7 another, 8 its own, and its state resets are read | the event-box loop body at `0x2a9ac4`, which is where the behaviour actually is. Still the biggest gap and the one that most changes how the game plays |
