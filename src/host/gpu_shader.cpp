...
ShaderCode SelectOverlay(SDL_GPUShaderFormat format, bool vertex) {
  using namespace embedded;
  if (format == SDL_GPU_SHADERFORMAT_SPIRV)
    return vertex ? ShaderCode{overlay_vert_spv, sizeof(overlay_vert_spv),
                               format, "main"}
                  : ShaderCode{overlay_frag_spv, sizeof(overlay_frag_spv),
                               format, "main"};
  if (format == SDL_GPU_SHADERFORMAT_DXIL)
    return vertex ? ShaderCode{overlay_vert_dxil, sizeof(overlay_vert_dxil),
                               format, "main"}
                  : ShaderCode{overlay_frag_dxil, sizeof(overlay_frag_dxil),
                               format, "main"};
  return vertex ? ShaderCode{overlay_vert_msl, sizeof(overlay_vert_msl), format,
                             "main0"}
                : ShaderCode{overlay_frag_msl, sizeof(overlay_frag_msl), format,
                             "main0"};
}

ShaderCode SelectUi(SDL_GPUShaderFormat format, bool vertex) {
  using namespace embedded;
  if (format == SDL_GPU_SHADERFORMAT_SPIRV)
    return vertex ? ShaderCode{ui_vert_spv, sizeof(ui_vert_spv), format, "main"}
                  : ShaderCode{ui_frag_spv, sizeof(ui_frag_spv), format,
                               "main"};
  if (format == SDL_GPU_SHADERFORMAT_DXIL)
    return vertex ? ShaderCode{ui_vert_dxil, sizeof(ui_vert_dxil), format,
                               "main"}
                  : ShaderCode{ui_frag_dxil, sizeof(ui_frag_dxil), format,
                               "main"};
  return vertex ? ShaderCode{ui_vert_msl, sizeof(ui_vert_msl), format, "main0"}
                : ShaderCode{ui_frag_msl, sizeof(ui_frag_msl), format,
                             "main0"};
}
