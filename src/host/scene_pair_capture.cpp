...
public:
  explicit Impl(std::string output_prefix)
      : prefix(std::move(output_prefix)), renderer(device), overlay(device) {
    if (prefix.empty())
...
void ScenePairCapture::WriteFromGles(const RenderSnapshot &snapshot,
  ...
  lucent::info(
      "gpu",
      "live snapshot pair: {} instances ({} skinned), {} cached assets, fade "
      "coverage {:.3f}; {} of {} pixels differ; wrote {} and {}",
      snapshot.instances.size(), snapshot.skinned_count(),
      impl_->renderer.cached_asset_count(), overlay.color[3], different,
      width * height, gl_path, gpu_path);
}
